...
/**
 * 최근 90일 기록에서 (메모 + 카테고리 + 계좌) 조합의 빈도를 세어
 * 자주 쓰는 항목을 뽑습니다. 금액은 가장 최근 값을 씁니다.
 * 자동 기록된 고정지출은 제외합니다 — 이미 알아서 들어가니까요.
 */
export function frequentEntries(type = 'expense', limit = 4) {
  const since = D.addDays(D.today(), -90);
  const map = new Map();

  for (const t of state.transactions) {
    if (t.type !== type) continue;
    if (t.date < since) continue;
    if (t.source === 'recurring') continue;
    const label = (t.memo || '').trim() || (category(t.categoryId) || {}).name || '';
    if (!label) continue;
    const key = `${label}|${t.categoryId}|${t.accountId}`;
    const prev = map.get(key);
    if (prev) {
      prev.count += 1;
      if (t.date > prev.date || (t.date === prev.date && t.createdAt > prev.createdAt)) {
  ...
  return [...map.values()]
    .filter((e) => e.count >= 2)
    .sort((a, b) => (b.count === a.count ? b.date.localeCompare(a.date) : b.count - a.count))
    .slice(0, limit);
}
