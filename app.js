...
/** 시트를 앱 안에서 열었는지 (주소로 바로 진입했는지) 구분하는 표시 */
let sheetFromApp = false;

function routeRoot(hash) {
  return (hash.replace(/^\//, '').split(/[/?]/)[0] || '').trim();
}

function go(hash, replace = false) {
  if (!replace && SHEET_ROOTS.has(routeRoot(hash))) sheetFromApp = true;
...
function back() {
  const r = parseRoute();
  const onSheet = SHEET_ROOTS.has(r.segs[0] || '');
  // 시트 주소로 바로 들어온 경우(새로고침·바로가기)에는 히스토리를 되감으면
  // 앱 밖으로 나가버리므로 기준 화면으로 바꿔치기합니다.
  if (onSheet && !sheetFromApp) {
    go(baseRoute.path || '/', true);
    return;
  }
...
let sheetState = null;
let currentSheetKey = '';
let baseRoute = { path: '/', segs: [], q: new URLSearchParams() };
...
function repaintSheet() {
  ...
  const host = $('#sheet');
  // 다시 그리기 전 스크롤 위치를 기억했다가 그대로 되돌립니다
  const prevBody = host.querySelector('.sheet-body');
  const prevScroll = prevBody ? prevBody.scrollTop : 0;

  host.innerHTML = `<div class="backdrop" data-act="close-sheet"></div>${html}`;

  // 내용 갱신은 '올라오는' 등장 애니메이션을 다시 재생하지 않습니다.
  // 이걸 빼면 키를 누를 때마다 시트가 덜컹거립니다.
  host.querySelectorAll('.sheet, .backdrop').forEach((el) => {
...
/**
 * 키패드 입력 전용 갱신.
 * 바뀌는 건 금액 숫자와 저장 버튼 상태뿐이라 그 두 곳만 손댑니다.
 * (시트 전체를 다시 그리면 DOM이 통째로 교체되면서 화면이 흔들립니다)
 */
function updateAmountOnly() {
  const s = sheetState;
  const total = entryTotal();
  const big = document.querySelector('.amount-display .big');
  if (!big) {
    repaintSheet();
    return;
  }
  const cap = document.querySelector('.amount-display .cap');
  if (cap) cap.textContent = `금액${s.pending ? ` · 이어서 더하는 중 (${won(s.pending)} +)` : ''}`;
  big.innerHTML = `${won(total)}<small>원</small>`;

  const save = document.querySelector('[data-act="save-entry"]');
  if (save) save.disabled = total <= 0;
}
