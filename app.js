...
const historyFilter = { type: '', q: '', categoryId: '' };
    ...
    <div class="body gap-8">
      ...
      <button class="menu-card" data-act="nav" data-to="/history">
        ...
        <div class="grow"><div class="t">거래내역</div><div class="s">${
          monthTxCount ? `${D.monthLabel(viewYm).split(' ')[1]} ${monthTxCount}건 · 검색과 필터` : '아직 기록이 없어요'
        }</div></div>
        ${icon('forward', 17, 2)}
      </button>
...
function viewHistory() {
  ...
  const list = S.transactions({
    ...
    query: historyFilter.q || undefined
  });
  ...
  <div class="screen">
    <header class="nav">
      ...
      <div class="nav-actions">
        <button class="icon-btn" data-act="toggle-search" aria-label="검색">${icon('search', 20, 1.9)}</button>
      </div>
    </header>
    ...
    <div class="body">
      ...
      <div id="search-wrap" ${historyFilter.q ? '' : 'hidden'}>
        <label class="field">
          ${icon('search', 17)}
          <input id="search-input" type="search" placeholder="메모 · 카테고리 · 금액 검색" value="${esc(historyFilter.q)}" data-act="search">
        </label>
      </div>
      ...
      ${
        ...
        <div>
          ...
          : `<div class="card"><div class="empty">${icon('receipt', 34, 1.4)}<b>${
              historyFilter.q || historyFilter.type || filterCat ? '조건에 맞는 기록이 없어요' : '이 달의 기록이 없어요'
...
function viewStats() {
  const ymList = statsPeriod === 'year' ? null : viewYm;
  const bd = S.categoryBreakdown(viewYm);
  const sum = S.monthSummary(viewYm);
  const prev = S.monthSummary(D.shiftMonth(viewYm, -1));
  const tr = S.trend(viewYm, 6);
  ...
  <div class="screen">
    ...
    <div class="body">
      <div class="section-head" style="padding:0">
        ...
        <div class="month-nav">
          <button class="icon-btn" style="width:32px;height:36px;color:var(--text-3)" data-act="month" data-d="-1" aria-label="이전 달">${icon('back', 16, 2)}</button>
          <b style="font-size:13px">${D.monthLabel(viewYm)}</b>
          <button class="icon-btn" style="width:32px;height:36px;color:var(--text-3)" data-act="month" data-d="1" aria-label="다음 달">${icon('forward', 16, 2)}</button>
        </div>
      </div>
      ...
      ${
        ...
        <div class="donut-wrap">
          <svg width="152" height="152" viewBox="0 0 152 152" role="img" aria-label="카테고리별 지출 비중">
            ...
            <text x="76" y="68" text-anchor="middle" font-size="10" fill="var(--text-2)">${
              D.monthLabel(viewYm).split(' ')[1]
            } 총 지출</text>
        ...
        ${
          diff === null
            ? ''
            : `<div style="text-align:center;margin-top:6px;font-size:11px;color:var(--text-2)">전월 대비 <b class="num" style="color:${
        ...
        <div style="display:flex;flex-direction:column;gap:4px;margin-top:12px">
          ...
          : `<div class="card"><div class="empty">${icon('chart', 34, 1.4)}<b>분석할 지출이 없어요</b><p>지출을 몇 건 기록하면<br>카테고리별 비중을 보여드립니다.</p></div></div>`
      }

      <div class="card pad">
        <div class="section-head" style="padding:0">
          <span style="font-size:12px;font-weight:700">월별 지출 추이</span>
          <span style="font-size:10px;color:var(--text-2)">최근 6개월</span>
        </div>
        <div class="trend">
          ${tr
            .map((x, i) => {
              const on = i === tr.length - 1;
              const h = Math.max(3, Math.round((x.amount / max) * 96));
              return `<div class="col ${on ? 'on' : ''}">${
                on && x.amount ? `<b class="num">${Math.round(x.amount / 10000)}만</b>` : ''
              }<i style="height:${h}px"></i></div>`;
            })
            .join('')}
        </div>
        <div class="trend-x">
          ${tr.map((x, i) => `<div class="${i === tr.length - 1 ? 'on' : ''}">${x.label}</div>`).join('')}
        </div>
      </div>

      <button class="card pad" data-act="nav" data-to="/budget" style="text-align:left;width:100%">
        <div class="section-head" style="padding:0">
          <span style="font-size:12px;font-weight:700">${D.monthLabel(viewYm).split(' ')[1]} 예산</span>
          <span class="num" style="font-size:11px;color:var(--text-2)">${
            sum.budget ? `${won(sum.expense)} / ${won(sum.budget)}` : '설정하기 →'
          }</span>
        </div>
        ${
          sum.budget
...
function sheetEntry() {
  ...
  const freq = s.mode === 'new' && !isTransfer ? S.frequentEntries(s.type, 4) : [];
  ...
  <section class="sheet full" role="dialog" aria-label="${s.mode === 'edit' ? '거래 수정' : '내역 입력'}">
    ...
    <div class="sheet-body">
      ${
        freq.length
          ? `<div>
              <div class="form-label">자주 쓰는 항목</div>
              <div class="chips scroll">
                ${freq
                  .map((f, i) => {
                    const c = S.category(f.categoryId);
                    return `<button class="chip freq" data-act="use-freq" data-i="${i}">
                      <span class="dot" style="background:${catColor(c)}"></span>
                      <b>${esc(f.label)}</b><span class="num">${won(f.amount)}</span>
                    </button>`;
                  })
                  .join('')}
              </div>
...
document.addEventListener('click', (e) => {
  ...
  switch (act) {
    ...
    case 'toggle-search': {
      ...
      if (!wrap.hidden) $('#search-input').focus();
      else {
        historyFilter.q = '';
        renderScreen(parseRoute());
      }
    ...
    case 'use-freq': {
      const f = S.frequentEntries(s.type, 4)[Number(el.dataset.i)];
      if (f) {
        s.amount = String(f.amount);
        s.pending = 0;
